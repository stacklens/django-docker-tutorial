...
**数据卷有个很重要的特性**：启动时如果卷是**空的**，则会将容器映射目录的所有内容复制到卷里去。换句话说就是，只要卷初始化完成后，容器原始的 `collected_static` 目录就不会再使用了，新增的文件也只存在于卷中，容器中是没有的。

实际上 static 静态文件（以及 media 媒体文件）的持久存储，通过**挂载**或者**数据卷**都可以实现；具体用哪种，这个就见仁见智了，你自己选择。
